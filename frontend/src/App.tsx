  ...
  const loadInitialData = async () => {
    try {
      const [ticketsRes, callsRes, customersRes, agentsRes] = await Promise.all([
        ApiService.getTickets().catch(() => ({ success: false })),
        ApiService.getCalls().catch(() => ({ success: false })),
        ApiService.getCustomers().catch(() => ({ success: false })),
        ApiService.getLiveAgents().catch(() => ({ success: false })),
      ]);

      if (ticketsRes?.success && ticketsRes.data) {
        const list = ticketsRes.data.data || ticketsRes.data.tickets || (Array.isArray(ticketsRes.data) ? ticketsRes.data : []);
        setTickets(list);
      }
      if (callsRes?.success && callsRes.data) {
        const list = callsRes.data.data || callsRes.data.calls || (Array.isArray(callsRes.data) ? callsRes.data : []);
        setCalls(list);
      }
      if (customersRes?.success && customersRes.data) {
        const list = customersRes.data.data || customersRes.data.customers || (Array.isArray(customersRes.data) ? customersRes.data : []);
        setCustomers(list);
      }
